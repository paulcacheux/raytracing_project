...
pub fn default_scene_builder() -> SceneDescription {
    let default_preset = PresetConfig {
        width: 600,
        height: 400,
        look_from: Pt3::new(13.0, 2.0, 3.0),
        look_at: Pt3::new(0.0, 0.0, 0.0),
    ...
    let checker_texture = CheckerTexture::new(
        ...
        Arc::new(SolidTexture::new(Vec3::repeat(0.9))),
        10.0,
    );
    ...
    for a in -11..11 {
        for b in -11..11 {
            ...
            if (center - Vec3::new(4.0, 0.2, 0.0)).coords.norm() > 0.9 {
                let mat: FloatTy = rng.gen();

                if mat < 0.8 {
                    let albedo = Vec3::new_random().component_mul(&Vec3::new_random());
...
pub fn cornell_box() -> SceneDescription {
    let default_preset = PresetConfig {
        width: 400,
        height: 400,
        look_from: Pt3::new(278.0, 278.0, -800.0),
        look_at: Pt3::new(278.0, 278.0, 0.0),
        up: Vec3::new(0.0, 1.0, 0.0),
        vfov: 40.0,
        sample_count: 40,
        max_depth: 5,
    ...
    let wall = Arc::new(Lambertian::from_solid_color(Vec3::repeat(1.0)));
    ...
    let mirror = Arc::new(Metal::new(Vec3::repeat(0.9), None));
